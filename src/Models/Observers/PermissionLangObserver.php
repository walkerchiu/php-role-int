...
namespace WalkerChiu\Role\Models\Observers;

class PermissionLangObserver
{
    /**
    ...
    {
        $query =
            config('wk-core.class.role.permissionLang')
                ::withTrashed()
        ...
        if (
            config('wk-role.soft_delete')
            && (
                config('wk-core.lang_log')
                || config('wk-role.lang_log')
            )
        ) {
